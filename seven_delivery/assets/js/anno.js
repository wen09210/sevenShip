...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.Anno = Anno = (function() {
	  ...
	  function Anno(arg) {
	    ...
	    if (options instanceof Anno) {
	     console.log('Anno constructor parameter is already an Anno object.');
	    }
	    if (options == null) {
	     console.log("new Anno() created with no options. It's recommended" + " to supply at least target and content.");
	    }
	    for (key in options) {
	      val = options[key];
	      if (key === 'chainTo' || key === 'start' || key === 'show' || key === 'hide' || key === 'hideAnno' || key === 'chainSize' || key === 'chainIndex' || key === 'version') {
	       console.log(("Anno: Overriding '" + key + "' is not recommended. Can ") + "you override a delegated function instead?");
	      }
	    }
	    for (key in options) {
	      val = options[key];
	  ...
	  Anno.chain = function(array) {
	   console.log('Anno.chain([...]) is deprecated. Use ' + '`new Anno([...])` instead.');
	  ...
	  Anno.prototype.show = function() {
	    ...
	    if (this._annoElem != null) {
	     console.log(("Anno elem for '" + this.target + "' has already been ") + "generated.  Did you call show() twice?");
	    }
	    this._annoElem = this.annoElem();
	    this.emphasiseTarget();
	    this.showOverlay();
	    $target.before(this._annoElem);
	  ...
	  Anno.prototype.hideAnno = function() {
	    if (this._annoElem != null) {
	     ...
	     console.log(("Can't hideAnno() for '" + this.target + "' when @_annoElem ") + "is null.  Did you call hideAnno() twice?");
	    }
	    return this;
	  };
	  ...
	  Anno.prototype.switchTo = function(otherAnno) {
	    if (otherAnno != null) {
	     ...
	     console.log("Can't switchTo a null object. Hiding instead.");
	      return this.hide();
	    }
	  };
	  ...
	  Anno.prototype.targetFn = function() {
	    ...
	    if (typeof this.target === 'string') {
	      ...
	      if (r.length > 1) {
	       console.log(("Anno target '" + this.target + "' matched " + r.length + " ") + "elements. Targeting the first one.");
	      }
	      return r.first();
	    } else if (this.target instanceof jQuery) {
	      if (this.target.length > 1) {
	       console.log(("Anno jQuery target matched " + this.target.length + " ") + "elements. Targeting the first one.");
	      }
	  ...
	  Anno.prototype.annoElem = function() {
	    ...
	    this._annoElem.find('.anno-inner').append(this.buttonsElem()).append(this.contentElem());
	  ...
	  Anno.prototype.showOverlay = function() {
	    ...
	    if ($('.anno-overlay').length === 0) {
	      $('body').before($e = this.overlayElem().addClass('anno-hidden'));
	      return setTimeout((function() {
	  ...
	  Anno.prototype.emphasiseTarget = function($target) {
	    ...
	    if ($target.css('backgroundColor') === 'rgba(0, 0, 0, 0)' || $target.css('backgroundColor') === 'transparent') {
	      console.log(("Anno.js target '" + this.target + "' has a transparent bg; ") + "filling it white temporarily.");
	  ...
	  Anno.prototype.positionFn = function() {
	    ...
	    if (this.position != null) {
	      ...
	      if (allowed.length === 0) {
	       ...
	       console.log(("Anno: guessing position:'" + allowed[0] + "' for ") + ("'" + this.target + "'. Possible Anno.preferredPositions: [" + allowed + "]."));
	      }
	  ...
	  Anno.prototype.arrowPositionFn = function() {
	    ...
	    if (this.arrowPosition != null) {
	     ...
	     console.log(("Guessing arrowPosition:'" + r + "' for " + this.target + ". ") + "Include this in your constructor for consistency.");
	      return r;
	    }
	  };
	...
	exports.AnnoButton = AnnoButton = (function() {
	  ...
	  AnnoButton.prototype.buttonElem = function(anno) {
	    return $("<div class='anno-btn'></div>").html(this.textFn(anno)).addClass(this.className).click((function(_this) {
	      return function(evt) {
